#!/usr/bin/env bash
# Run the hidden-burn demo against a self-managed local v8 stack.
#
...
# waits for it to be healthy, discovers the mapped ports, and points the demo at
# them via HB_*_PORT (see the `injected` branch in src/hidden-burn.ts).
#
# Usage:  bash scripts/run-hidden-burn.sh
set -euo pipefail

cd "$(dirname "$0")/.."
export TESTCONTAINERS_UID="${TESTCONTAINERS_UID:-hb}"
export NETWORK_ID="${NETWORK_ID:-undeployed}"
COMPOSE=(docker compose -f compose.yml)

cleanup() {
  echo "[run-hidden-burn] tearing down stack..."
  "${COMPOSE[@]}" down -v >/dev/null 2>&1 || true
}
trap cleanup EXIT

echo "[run-hidden-burn] bringing up node + indexer + proof-server..."
"${COMPOSE[@]}" up -d

wait_healthy() {
  local svc="$1" name="${2}" tries="${3:-60}"
  echo "[run-hidden-burn] waiting for $svc ($name) to be healthy..."
  for ((i = 1; i <= tries; i++)); do
    ...
    if [[ "$status" == "healthy" ]]; then
      echo "[run-hidden-burn]   $svc healthy after ${i}0s"
      return 0
    fi
    if [[ "$status" == "exited" || "$status" == "dead" ]]; then
      echo "[run-hidden-burn]   $svc $status — logs:"; docker logs "$name" 2>&1 | tail -30
      return 1
    fi
    sleep 10
  done
  echo "[run-hidden-burn]   $svc not healthy after $((tries * 10))s"; docker logs "$name" 2>&1 | tail -30
  return 1
}
...
wait_log() {
  ...
  echo "[run-hidden-burn] waiting for $name log /$pattern/..."
  for ((i = 1; i <= tries; i++)); do
    if docker logs "$name" 2>&1 | grep -qE "$pattern"; then
      echo "[run-hidden-burn]   $name ready after $((i * 5))s"
      return 0
    fi
    local status
    status=$(docker inspect --format '{{.State.Status}}' "$name" 2>/dev/null || echo missing)
    if [[ "$status" == "exited" || "$status" == "dead" ]]; then
      echo "[run-hidden-burn]   $name $status — logs:"; docker logs "$name" 2>&1 | tail -30; return 1
    fi
    sleep 5
  done
  echo "[run-hidden-burn]   $name log not seen after $((tries * 5))s"; docker logs "$name" 2>&1 | tail -30; return 1
}
...
echo "[run-hidden-burn] ports -> node=$HB_NODE_PORT indexer=$HB_INDEXER_PORT proof-server=$HB_PS_PORT"

echo "[run-hidden-burn] running the demo..."
pnpm hidden-burn
